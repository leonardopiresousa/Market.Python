...
def registar_produto() -> None:
    ...
    nome: str = input('Informa o nome do produto: ')
...
def comprar_produto() -> None:
    if len(produtos) > 0:
        ...
        produto: Produto = seleciona_produto_por_codigo(codigo)
    ...
    if len(carrinho) > 0:
        ...
        for item in carrinho:
            for dados in item.items():
                ...
                print('-----------------------')
                sleep(1)
        print(f'A tua fatura é de {formata_float_str_moeda(valor_total)}')
        print('Volta sempre! Obrigado')
        carrinho.clear()
        sleep(5)
...
def seleciona_produto_por_codigo(codigo: int) -> Produto:
    p: Produto = None
